...
export default function index({ data }) {
    ...
    return (
        <Layout title={"1337 Restaurant | Lunch"}>
            <MealPage mealName="lunch" data={data.data} />
        </Layout>
    )
}
...
export const getServerSideProps = requireAuthentication(async context => {
    ...
    const data = await axiosService(`${process.env.NEXT_PUBLIC_SERVER_API_URL}meals/lunch/`, cookies.token);
