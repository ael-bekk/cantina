...
import DashboardLogs from "../../../srcs/components/DashboardLogs";

export default function Logs({ data, token }) {
    const { status } = data;
    ...
    return (
        <Layout title={"1337 Restaurant | Closes"}>
            <DashboardLogs token={token} data={data.data} />
        </Layout>
    );
}
...
export const getServerSideProps = requireAuthentication(async (context) => {
    ...
    const data = await axiosService(
        `${process.env.NEXT_PUBLIC_SERVER_API_URL}hooks/logs/get/`,
        cookies.token
    );
